...
from .Visualization3d import _get_trajectory_points
...
class DashSimulator:
    """
    Dash/Plotly tabanli 3D drone simulasyonu.
    Tarayicida interaktif calisir: hedef koordinat gir, Git! butonuna bas.
    """
    ...
    def __init__(self, start_pos, obstacles, bounds, param=None):
        ...
        self.obstacle_data = self._precompute_obstacles()
    ...
    def plan_trajectory(self, start, goal):
        result = optimal_traj(
            start, goal, self.obstacles, self.bounds, **self.param
        )
        opt, Ts, op_wp, opt_hpolys, obstacles, bounds = result
        ...
        colors = [
            f'hsla({int(i * 360 / max(n_poly, 1))}, 70%, 60%, 0.2)'
    ...
    def _build_figure(self, drone_pos, trail_x, trail_y, trail_z,
                      poly_data=None, status="Hazir"):
        import plotly.graph_objects as go

        traces = []

        for oi, od in enumerate(self.obstacle_data):
    ...
    def _build_app(self):
        ...
        app.layout = html.Div([
            html.Div([
                html.H2("DDoP Drone Simulator",
                         style={'margin': '0', 'color': '#2c3e50'}),
                html.Div([
                    html.Div([
                        html.Label("X:", style={'fontWeight': 'bold'}),
                        dcc.Input(id='target-x', type='number', value=9.0,
                                  step=0.5, style={'width': '70px', 'marginRight': '10px'}),
                    ], style={'display': 'inline-block'}),
                    html.Div([
                        html.Label("Y:", style={'fontWeight': 'bold'}),
                        dcc.Input(id='target-y', type='number', value=4.0,
                                  step=0.5, style={'width': '70px', 'marginRight': '10px'}),
                    ], style={'display': 'inline-block'}),
                    html.Div([
                        html.Label("Z:", style={'fontWeight': 'bold'}),
                        dcc.Input(id='target-z', type='number', value=4.0,
                                  step=0.5, style={'width': '70px', 'marginRight': '10px'}),
                    ], style={'display': 'inline-block'}),
            ...
            dcc.Store(id='trace-indices', data={
                'trail': n_obs, 'drone': n_obs + 1
            }),
        ...
        def on_go(n_clicks, tx, ty, tz, drone_state):
            ...
            lower, upper = sim.bounds
            for i, (ax, lo, hi) in enumerate(zip(['X', 'Y', 'Z'], lower, upper)):
                if goal[i] < lo or goal[i] > hi:
                    return (no_update, None, 0, True,
                            f"{ax}={goal[i]:.1f} sinir disinda!", no_update)

            try:
                result = sim.plan_trajectory(start, goal)
            except Exception as e:
                return no_update, None, 0, True, f"Hata: {e}", no_update

            poly_data = result.pop('poly_data')
            n_poly = len(poly_data)

            trail_idx = n_obs + n_poly
        ...
        def on_reset(n):
            ...
            state = {
                'pos': pos,
                'trail_x': [pos[0]], 'trail_y': [pos[1]], 'trail_z': [pos[2]],
            }
            fig = sim._build_figure(pos, [pos[0]], [pos[1]], [pos[2]], None, "Reset!")
            indices = {'trail': n_obs, 'drone': n_obs + 1}
        ...
        def animate(n_intervals, drone_state, traj_data, anim_idx, indices, speed):
            ...
            new_state = {
                'pos': [cx, cy, cz],
                'trail_x': trail_x, 'trail_y': trail_y, 'trail_z': trail_z,
            }
    ...
    @staticmethod
    def start(start_pos, obstacles, bounds, param=None, port=8050):
        sim = DashSimulator(start_pos, obstacles, bounds, param)
        sim.run(port=port)
